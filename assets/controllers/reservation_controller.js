...
export default class extends Controller {
    ...
    getHTMLCard(card) {
        ...
        card.accomodation.tags.forEach(
            (tag) =>
                (tags += `<div class='bg-main-100 rounded-full p-1 px-4 text-sm'>${tag}</div>`)
        );

        return `
                <article id="accom-${card.accomodation.id}" data-location-id="${
                    card.location.id
                }" data-location-number="${
                    card.location.number
                }" class="reservation-card available-reservation bg-white border border-gray-200 p-2 text-black">
                    <section class="flex gap-4">
                        <div class="image-container w-1/3 aspect-[16/12] border border-white outline-1 outline-gray-200">
                            <img class="h-full" src="uploads/accomodations/${
                                card.accomodation.image
                            }">
                        </div>
                        <div class="description-container flex flex-col relative flex-1 mt-2">
                            <div class="text-lg font-title">${
                                card.accomodation.name
                            }</div>
                            ...
                            <div class="flex items-center self-end place-self-end gap-2 w-fit h-fit mt-auto">
                                <p class="text-xs opacity-80">prix pour ${
                                    card.reservation.stay
                                } nuits,</p>
                                <button data-action="click->reservation#price" data-div="price-${
                                    card.accomodation.id
                                }" class="text-xs opacity-80 underline hover:opacity-100">voir le détail du prix</button>
                                <button class='bg-main-400 text-white rounded-full py-2 px-4'>${
                                    card.price.total
                    ...
                    <div id="price-${
                        ...
                        Du ${card.reservation.start} au ${
                            card.reservation.end
                        } (${card.reservation.stay} nuits x ${
                            card.price.accomodation
                        }€) : ${
                            card.price.accomodation * card.reservation.stay
                        }€ <br>
                        <span class="font-bold"> Adultes supplémentaires (${
                            card.reservation.adults
                        }) : ${card.price.adult}€ </span> <br>
                        Du ${card.reservation.start} au ${
                            card.reservation.end
                        } (${card.reservation.stay} nuits x ${
                            card.reservation.adults
                        ...
                        <span class="font-bold"> Enfants supplémentaires (${
                            card.reservation.childs
                        }) : ${card.price.child}€ </span> <br>
                        Du ${card.reservation.start} au ${
                            card.reservation.end
                        } (${card.reservation.stay} nuits x ${
                            card.reservation.childs
